...
function getStoriesMenu(lang) {
	...
	for(var index = 0; index < stories.length; index++) {
		...
		title.on(events['tap'], function (evt) {
...
function createStoryAlter(story) {
	...
	if(nbSentences > maxVisibleLines) {
		/*storyGroup.gesture = new Gesture(storyGroup);
			storyGroup.on("scrollUp", function () {
				storyGroup.setY( storyGroup.getY() + (gesture.getVelocity() / storyGroup.getHeight()) );
			});
			storyGroup.on("scrollDown", function () {
				storyGroup.setY( storyGroup.getY() - (gesture.getVelocity() / storyGroup.getHeight()) );
			});
		*/
	}
