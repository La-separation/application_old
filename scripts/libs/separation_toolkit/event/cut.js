...
Event.onCut = function(id, object, callback, callbackOnChange, restart) {
	Event.cut_obj[id] = {
	...
	// Event.cut_obj[id].rect = new Kinetic.Rect({
			// x:Event.cut_obj[id].x1,
			// y:Event.cut_obj[id].y1,
			// width:Event.cut_obj[id].x2-Event.cut_obj[id].x1,
			// height:Event.cut_obj[id].y2-Event.cut_obj[id].y1,
			// fill:'#a00',
			// opacity: 0.5,
		// });
	// mainLayer.add(Event.cut_obj[id].rect);
	// mainLayer.draw();
}

Event.cut = function(event) {
	event.preventDefault();
	var coords = getTouchMove(event);
	for(var i in Event.cut_obj) {
		var obj = Event.cut_obj[i];
		...
		if((obj.y1 <= coords.y1) && (coords.y1 <= obj.y2) &&
		   (obj.y1 <= coords.y2) && (coords.y2 <= obj.y2)) {
			...
			// c'est le début d'un cut ltr (left to right)
			...
			// c'est le début d'un cut rtl (right to left)
			else if((coords.x2 <= obj.x2) && (obj.x2 <= coords.x1)) {
				obj.direction = -1;
				obj.onChange(obj.direction, obj.value);
			}

			// Si un cut est en cours
			// On calcule la valeur de l'avancement du geste
			if(obj.direction == 1) { // Si c'est un cut ltr
				obj.value = (coords.x2 - obj.x1) / (obj.x2 - obj.x1);
			}
			else if(obj.direction == -1) { // Si c'est un cut rtl
